...
// INCLUDES                                                                            //
// =================================================================================== //
#include "sphexa_common.hpp"
#if SPHEXA_ENABLE_MPI==1
#include "sphexa_communications.hpp"
#endif
...
namespace sphexa {
    ...
    */
    ...
    /*!
        \struct ParTree::LoadBalanceRanges
        \ingroup SPHEXA
    ...
    */
    ...
    /*!
        \enum ParTree::LoadBalanceRanges::ExchangeAction
        \ingroup SPHEXA
    ...
    */
    ...
    /*! Default constructor
    */
    ParTree::LoadBalanceRanges::LoadBalanceRanges()
     ...
     */
    ParTree::LoadBalanceRanges::LoadBalanceRanges(bool serial, const ExchangeRanges &_sendRanges, const ExchangeRanges &_recvRanges)
    ...
    /*! Clear the ranges
     */
    void ParTree::LoadBalanceRanges::clear()
    {
    ...
    const std::string	ParTree::DEFAULT_LOG_FILE   = "SPHEXA";

    // =================================================================================== //
    // CONSTRUCTORS AND OPERATORS														   //
    // =================================================================================== //

    /*! Default empty constructor of ParTree.
     ...
     */
    ParTree::ParTree(std::string logfile, MPI_Comm comm )
#else
    ParTree::ParTree(std::string logfile )
#endif
    {
#if SPHEXA_ENABLE_MPI==1
    ...
    /*! Default constructor of ParTree.
     ...
     * \param[in] comm The MPI communicator used by the parallel octree. MPI_COMM_WORLD is the default value.
     */
    ParTree::ParTree(uint8_t dim, std::string logfile, MPI_Comm comm )
#else
    ParTree::ParTree(uint8_t dim, std::string logfile )
#endif
     ...
     * \param[in] comm The MPI communicator used by the parallel octree. MPI_COMM_WORLD is the default value.
     */
    ParTree::ParTree(std::istream &stream, std::string logfile, MPI_Comm comm)
#else
    ParTree::ParTree(std::istream &stream, std::string logfile)
#endif
    {
#if SPHEXA_ENABLE_MPI==1
        initialize(logfile, comm);
    ...
    /*!
     * Copy constructor of ParTree.
     * \param[in] other class of type ParTree
     */
    ParTree::ParTree(const ParTree & other)
     ...
     */
    void
    ParTree::_initializeCommunications(MPI_Comm comm) {
     ...
     */
    void
    ParTree::_initialize(uint8_t dim, const std::string &logfile) {
     ...
     */
    void
    ParTree::initialize(const std::string &logfile, MPI_Comm comm) {
#else
    void
    ParTree::initialize(const std::string &logfile) {
#endif
#if SPHEXA_ENABLE_MPI==1
    ...
    void
#if SPHEXA_ENABLE_MPI==1
    ParTree::initialize(uint8_t dim, const std::string &logfile, MPI_Comm comm) {
#else
    ParTree::initialize(uint8_t dim, const std::string &logfile) {
#endif
#if SPHEXA_ENABLE_MPI==1
        // Initialize communications
        _initializeCommunications(comm);
     ...
     */
    void
    ParTree::reinitialize(uint8_t dim, const std::string &logfile) {
        // Initialize partitions
        _initializePartitions();
     ...
     */
    void
    ParTree::initializeLogger(const std::string &logfile){
     ...
     */
    int ParTree::getDumpVersion() const
    {
     ...
     */
    void ParTree::dump(std::ostream &stream, bool full)
    {
        // Version
     ...
     */
    void ParTree::restore(std::istream &stream)
    {
        // Version
        int version;
    ...
    /*! Print the initial SPHEXA header.
     */
    void
    ParTree::printHeader(){
        (*m_log) << log::context("SPHEXA");
        (*m_log) << "---------------------------------------------" << endl;
        (*m_log) << "- SPHEXA PArallel Balanced Linear Octree -" << endl;
        (*m_log) << "---------------------------------------------" << endl;
        (*m_log) << " " << endl;
        (*m_log) << "---------------------------------------------" << endl;
        (*m_log) << "- SPHEXA restart -" << endl;
        (*m_log) << "---------------------------------------------" << endl;
    ...
    /*! Print the final SPHEXA footer.
     */
    void
    ParTree::printFooter(){
        (*m_log) << "---------------------------------------------" << endl;
        (*m_log) << "--------------- R.I.P. SPHEXA ----------------" << endl;
        (*m_log) << "---------------------------------------------" << endl;
        (*m_log) << "---------------------------------------------" << endl;
    }
     ...
     */
    uint64_t
    ParTree::getGlobalNumOctants() const {
     ...
     */
    ParTree::Operation
    ParTree::getLastOperation() const {
        return m_lastOp;
    }
     ...
     */
    void
    ParTree::setComm(MPI_Comm communicator)
    {
        // Communication can be set just once
        if (isCommSet()) {
            throw std::runtime_error ("SPHEXA communicator can be set just once");
        }

        // The communicator has to be valid
        if (communicator == MPI_COMM_NULL) {
            throw std::runtime_error ("SPHEXA communicator is not valid");
        }
     ...
     */
    void
    ParTree::replaceComm(MPI_Comm communicator, MPI_Comm *previousCommunicator)
    {
        // The communicator has to be already set
        if (!isCommSet()) {
            throw std::runtime_error ("SPHEXA communicator is not set");
        }
    ...
    ParTree::getPartitionRangeGlobalIdx() const {
     ...
     */
    uint8_t
    ParTree::getNnodesperface() const {
     ...
     */
    void
    ParTree::getNormals(int8_t normals[6][3]) const {
        for (int i=0; i<6; i++){
            for (int j=0; j<3; j++){
     ...
     */
    void
    ParTree::getOppface(uint8_t oppface[6]) const {
        for (int j=0; j<6; j++){
     ...
     */
    void
    ParTree::getFacenode(uint8_t facenode[6][4]) const {
        for (int i=0; i<6; i++){
            for (int j=0; j<4; j++){
     ...
     */
    void
    ParTree::getNodeface(uint8_t nodeface[8][3]) const {
        for (int i=0; i<8; i++){
            for (int j=0; j<3; j++){
     ...
     */
    void
    ParTree::getEdgeface(uint8_t edgeface[12][2]) const {
        for (int i=0; i<12; i++){
            for (int j=0; j<2; j++){
     ...
     */
    void
    ParTree::getNodecoeffs(int8_t nodecoeffs[8][3]) const {
        for (int i=0; i<8; i++){
            nodecoeffs[i][2] = 0;
     ...
     */
    void
    ParTree::getEdgecoeffs(int8_t edgecoeffs[12][3]) const {
        for (int i=0; i<12; i++){
            for (int j=0; j<3; j++){
     ...
     */
    bool
    ParTree::getPeriodic(uint8_t i) const {
        return m_periodic[i];
    };
     ...
     */
    darray3
    ParTree::getCoordinates(uint32_t idx) const {
     ...
     */
    double
    ParTree::getX(uint32_t idx) const {
     ...
     */
    double
    ParTree::getY(uint32_t idx) const {
     ...
     */
    double
    ParTree::getZ(uint32_t idx) const {
     ...
     */
    double
    ParTree::getSize(uint32_t idx) const {
     ...
     */
    double
    ParTree::getArea(uint32_t idx) const {
     ...
     */
    double
    ParTree::getVolume(uint32_t idx) const {
     ...
     */
    void
    ParTree::getCenter(uint32_t idx, darray3& center) const {
     ...
     */
    darray3
    ParTree::getCenter(uint32_t idx) const {
        darray3 center;
        darray3 center_ = m_octree.m_octants[idx].getCenter();
        m_trans.mapCenter(center_, center);
        return center;
     ...
     */
    darray3
    ParTree::getFaceCenter(uint32_t idx, uint8_t iface) const {
        darray3 center;
     ...
     */
    void
    ParTree::getFaceCenter(uint32_t idx, uint8_t iface, darray3& center) const {
        darray3 center_ = m_octree.m_octants[idx].getFaceCenter(iface);
        m_trans.mapCenter(center_, center);
    }
     ...
     */
    darray3
    ParTree::getNode(uint32_t idx, uint8_t inode) const {
        darray3 node;
     ...
     */
    void
    ParTree::getNode(uint32_t idx, uint8_t inode, darray3& node) const {
        u32array3 node_ = m_octree.m_octants[idx].getNode(inode);
        m_trans.mapNode(node_, node);
    }
     ...
     */
    void
    ParTree::getNodes(uint32_t idx, darr3vector & nodes) const {
     ...
     */
    darr3vector
    ParTree::getNodes(uint32_t idx) const{
        darr3vector nodes;
        u32arr3vector nodes_;
        m_octree.m_octants[idx].getNodes(nodes_);
        m_trans.mapNodes(nodes_, nodes);
     ...
     */
    void
    ParTree::getNormal(uint32_t idx, uint8_t iface, darray3 & normal) const {
        i8array3 normal_;
     ...
     */
    darray3
    ParTree::getNormal(uint32_t idx, uint8_t iface) const {
        darray3 normal;
        i8array3 normal_;
        m_octree.m_octants[idx].getNormal(iface, normal_, m_global.m_normals);
        m_trans.mapNormals(normal_, normal);
     ...
     */
    int8_t
    ParTree::getMarker(uint32_t idx) const {
    ...
    */
    int8_t
    ParTree::getPreMarker(uint32_t idx){
     ...
     */
    uint8_t
    ParTree::getLevel(uint32_t idx) const {
     ...
     */
    uint64_t
    ParTree::getMorton(uint32_t idx) const {
     ...
     */
    uint64_t
    ParTree::getNodeMorton(uint32_t idx, uint8_t inode) const {
     ...
     */
    bool
    ParTree::getBalance(uint32_t idx) const {
     ...
     */
    bool
    ParTree::getBound(uint32_t idx, uint8_t iface) const {
     ...
     */
    bool
    ParTree::getBound(uint32_t idx) const {
     ...
     */
    bool
    ParTree::getPbound(uint32_t idx, uint8_t iface) const {
     ...
     */
    bool
    ParTree::getPbound(uint32_t idx) const {
     ...
     */
    bool
    ParTree::getIsNewR(uint32_t idx) const {
     ...
     */
    bool
    ParTree::getIsNewC(uint32_t idx) const {
     ...
     */
    uint64_t
    ParTree::getGlobalIdx(uint32_t idx) const {
        if (m_rank){
     ...
     */
    uint32_t
    ParTree::getLocalIdx(uint64_t gidx,int rank) const {
        if (rank){
     ...
     */
    uint32_t
    ParTree::getLocalIdx(uint64_t gidx) const {
        if (m_rank){
     ...
     */
    uint32_t
    ParTree::getGhostLocalIdx(uint64_t gidx) const {
     ...
     */
    uint64_t
    ParTree::getGhostGlobalIdx(uint32_t idx) const {
     ...
     */
    bitset<72>
    ParTree::getPersistentIdx(uint32_t idx) const {
     ...
     */
    void
    ParTree::setMarker(uint32_t idx, int8_t marker){
        if (m_lastOp == OP_PRE_ADAPT) {
     ...
     */
    void
    ParTree::setBalance(uint32_t idx, bool balance){
        if (m_lastOp == OP_PRE_ADAPT) {
            throw std::runtime_error("It is not possible to update the tree until the adaption is completed");
        }
     ...
     */
    darray3
    ParTree::getCoordinates(const Octant* oct) const {
     ...
     */
    double
    ParTree::getX(const Octant* oct) const {
     ...
     */
    double
    ParTree::getY(const Octant* oct) const {
     ...
     */
    double
    ParTree::getZ(const Octant* oct) const {
     ...
     */
    double
    ParTree::getSize(const Octant* oct) const {
     ...
     */
    double
    ParTree::getArea(const Octant* oct) const {
     ...
     */
    double
    ParTree::getVolume(const Octant* oct) const {
     ...
     */
    void
    ParTree::getCenter(const Octant* oct, darray3& center) const {
     ...
     */
    darray3
    ParTree::getCenter(const Octant* oct) const {
        darray3 center;
        darray3 center_ = oct->getCenter();
        m_trans.mapCenter(center_, center);
        return center;
     ...
     */
    darray3
    ParTree::getFaceCenter(const Octant* oct, uint8_t iface) const {
        darray3 center;
     ...
     */
    void
    ParTree::getFaceCenter(const Octant* oct, uint8_t iface, darray3& center) const {
        darray3 center_ = oct->getFaceCenter(iface);
        m_trans.mapCenter(center_, center);
    }
     ...
     */
    darray3
    ParTree::getNode(const Octant* oct, uint8_t inode) const {
        darray3 node;
     ...
     */
    void
    ParTree::getNode(const Octant* oct, uint8_t inode, darray3& node) const {
        u32array3 node_ = oct->getNode(inode);
        m_trans.mapNode(node_, node);
    }
     ...
     */
    void
    ParTree::getNodes(const Octant* oct, darr3vector & nodes) const {
        u32arr3vector nodes_;
        oct->getNodes(nodes_);
        m_trans.mapNodes(nodes_, nodes);
    }
     ...
     */
    darr3vector
    ParTree::getNodes(const Octant* oct) const {
        darr3vector nodes;
        u32arr3vector nodes_;
        oct->getNodes(nodes_);
        m_trans.mapNodes(nodes_, nodes);
     ...
     */
    void
    ParTree::getNormal(const Octant* oct, uint8_t iface, darray3 & normal) const {
        i8array3 normal_;
     ...
     */
    darray3
    ParTree::getNormal(const Octant* oct, uint8_t iface) const {
        darray3 normal;
        i8array3 normal_;
        oct->getNormal(iface, normal_, m_global.m_normals);
        m_trans.mapNormals(normal_, normal);
     ...
     */
    int8_t
    ParTree::getMarker(const Octant* oct) const {
     ...
     */
    int8_t
    ParTree::getPreMarker(Octant* oct){
        if (m_lastOp != OP_PRE_ADAPT) {
            throw std::runtime_error("Last operation different from preadapt, unable to call getPreMarker function");
        }
     ...
     */
    uint8_t
    ParTree::getLevel(const Octant* oct) const {
        return oct->getLevel();
    };
     ...
     */
    uint64_t
    ParTree::getMorton(const Octant* oct) const {
     ...
     */
    uint64_t
    ParTree::getNodeMorton(const Octant* oct, uint8_t inode) const {
     ...
     */
    bool
    ParTree::getBalance(const Octant* oct) const {
     ...
     */
    bool
    ParTree::getBound(const Octant* oct, uint8_t iface) const {
     ...
     */
    bool
    ParTree::getBound(const Octant* oct) const {
        return oct->getBound();
    }
     ...
     */
    bool
    ParTree::getPbound(const Octant* oct, uint8_t iface) const {
     ...
     */
    bool
    ParTree::getPbound(const Octant* oct) const {
     ...
     */
    bool
    ParTree::getIsNewR(const Octant* oct) const {
     ...
     */
    bool
    ParTree::getIsNewC(const Octant* oct) const {
     ...
     */
    uint32_t
    ParTree::getIdx(const Octant* oct) const {
#if SPHEXA_ENABLE_MPI==1
        if (getIsGhost(oct)){
     ...
     */
    uint64_t
    ParTree::getGlobalIdx(const Octant* oct) const {
#if SPHEXA_ENABLE_MPI==1
        if (getIsGhost(oct)){
     ...
     */
    bitset<72>
    ParTree::getPersistentIdx(const Octant* oct) const {
     ...
     */
    void
    ParTree::setMarker(Octant* oct, int8_t marker){
        if (m_lastOp == OP_PRE_ADAPT) {
            throw std::runtime_error("It is not possible to update the tree until the adaption is completed");
        }
     ...
     */
    void
    ParTree::setBalance(Octant* oct, bool balance){
        if (m_lastOp == OP_PRE_ADAPT) {
            throw std::runtime_error("It is not possible to update the tree until the adaption is completed");
        }
     ...
     */
    uint8_t
    ParTree::getLocalMaxDepth() const{
     ...
     */
    uint8_t
    ParTree::getBalanceCodimension() const{
     ...
     */
     uint64_t ParTree::getFirstDescMorton() const {
     ...
     */
     uint64_t ParTree::getLastDescMorton() const {
     ...
     */
    uint64_t
    ParTree::getLastDescMorton(uint32_t idx) const {
     ...
     */
    octantIterator
    ParTree::getInternalOctantsBegin() {
     ...
     */
    void
    ParTree::setBalanceCodimension(uint8_t b21codim){
        if (m_lastOp == OP_PRE_ADAPT) {
            throw std::runtime_error("It is not possible to update the tree until the adaption is completed");
        }
     ...
     */
    uint32_t
    ParTree::getNumIntersections() const {
     ...
     */
    Intersection*
    ParTree::getIntersection(uint32_t idx) {
     ...
     */
    uint8_t
    ParTree::getLevel(const Intersection* inter) const {
     ...
     */
    bool
    ParTree::getFiner(const Intersection* inter) const {
     ...
     */
    bool
    ParTree::getBound(const Intersection* inter) const {
     ...
     */
    bool
    ParTree::getIsGhost(const Intersection* inter) const {
     ...
     */
    bool
    ParTree::getPbound(const Intersection* inter) const {
     ...
     */
    uint8_t
    ParTree::getFace(const Intersection* inter) const {
     ...
     */
    u32vector
    ParTree::getOwners(const Intersection* inter) const {
        u32vector owners(2);
     ...
     */
    uint32_t
    ParTree::getIn(const Intersection* inter) const {
        return inter->getIn();
    }
     ...
     */
    uint32_t
    ParTree::getOut(const Intersection* inter) const {
        return inter->getOut();
    }
     ...
     */
    bool
    ParTree::getOutIsGhost(const Intersection* inter) const {
     ...
     */
    double
    ParTree::getSize(const Intersection* inter) const {
        uint32_t Size;
        if(inter->m_finer && inter->m_isghost)
     ...
     */
    double
    ParTree::getArea(const Intersection* inter) const {
        uint64_t Area;
        if(inter->m_finer && inter->m_isghost)
     ...
     */
    darray3
    ParTree::getCenter(const Intersection* inter) const {
        darray3 center;
        Octant oct(m_dim);
        if(inter->m_finer && inter->m_isghost)
     ...
     */
    darr3vector
    ParTree::getNodes(const Intersection* inter) const {
        darr3vector nodes;
        Octant oct(m_dim);
        if(inter->m_finer && inter->m_isghost)
            oct = m_octree.extractGhostOctant(inter->m_owners[inter->m_finer]);
     ...
     */
    darray3
    ParTree::getNormal(const Intersection* inter) const {
        darray3 normal;
        Octant oct(m_dim);
        if(inter->m_finer && inter->m_isghost)
            oct = m_octree.extractGhostOctant(inter->m_owners[inter->m_finer]);
     ...
     */
    Octant*
    ParTree::getOctant(uint32_t idx) {
     ...
     */
    const Octant*
    ParTree::getOctant(uint32_t idx) const {
        return (m_octree.m_octants.data() + idx);
    };
     ...
     */
    Octant*
    ParTree::getGhostOctant(uint32_t idx) {
     ...
     */
    const Octant*
    ParTree::getGhostOctant(uint32_t idx) const {
        return (m_octree.m_ghosts.data() + idx);
    };
     ...
     */
    bool
    ParTree::getIsGhost(const Octant* oct) const {
     ...
     */
    const ParTree::LoadBalanceRanges &
    ParTree::getLoadBalanceRanges() const {
    ...
    void
    ParTree::findNeighbours(const Octant* oct, bool haveIidx, uint32_t idx, uint8_t iface, uint8_t codim, u32vector & neighbours, bvector & isghost, bool onlyinternal) const{
     ...
     * \param[out] isghost Vector with boolean flag; true if the respective octant in neighbours is a ghost octant. Can be ignored in serial runs. */
    void
    ParTree::findNeighbours(uint32_t idx, uint8_t iface, uint8_t codim, u32vector & neighbours, bvector & isghost) const {
     ...
     * \param[out] isghost Vector with boolean flag; true if the respective octant in neighbours is a ghost octant. Can be ignored in serial runs. */
    void
    ParTree::findNeighbours(Octant* oct, uint8_t iface, uint8_t codim, u32vector & neighbours, bvector & isghost) const {
     ...
     */
    void
    ParTree::findGhostNeighbours(uint32_t idx, uint8_t iface, uint8_t codim, u32vector & neighbours) const {
     ...
     */
    void
    ParTree::findGhostNeighbours(uint32_t idx, uint8_t iface, uint8_t codim, u32vector & neighbours, bvector & isghost) const {
    ...
    */
    void
    ParTree::findAllNodeNeighbours(Octant* oct, uint32_t inode, u32vector & neighbours, bvector & isghost) const {
    ...
    */
    void
    ParTree::findAllNodeNeighbours(uint32_t idx, uint32_t inode, u32vector & neighbours, bvector & isghost) {
     ...
     */
    Octant*
    ParTree::getPointOwner(const dvector &point){
     ...
     */
    Octant*
    ParTree::getPointOwner(const dvector &point, bool & isghost){
     ...
     */
    Octant*
    ParTree::getPointOwner(const darray3 &point){
        uint32_t idx = getPointOwnerIdx(point);
        if(idx < numeric_limits<uint32_t>::max())
            return &m_octree.m_octants[idx];
        else
     ...
     */
    Octant*
    ParTree::getPointOwner(const darray3 &point, bool & isghost){
        uint32_t idx = getPointOwnerIdx(point,isghost);
        if(idx < numeric_limits<uint32_t>::max())
            if(isghost)
                return &m_octree.m_ghosts[idx];
     ...
     */
    uint32_t
    ParTree::getPointOwnerIdx(const darray3 &point) const {
     ...
     */
    uint32_t
    ParTree::getPointOwnerIdx(const darray3 &point, bool & isghost) const {
     ...
     */
    uint32_t
    ParTree::getPointOwnerIdx(const dvector &point) const {
        assert(point.size() >= 3);
        return getPointOwnerIdx(point.data());
    };
     ...
     */
    uint32_t
    ParTree::getPointOwnerIdx(const dvector &point, bool & isghost) const {
        assert(point.size() >= 3);
        return getPointOwnerIdx(point.data(),isghost);
    };
     ...
     */
    uint32_t
    ParTree::getPointOwnerIdx(const double * point) const {
     ...
     */
    uint32_t
    ParTree::getPointOwnerIdx(const double * point, bool & isghost) const {
        uint32_t noctants = m_octree.m_octants.size();
        if(noctants==0)
            return numeric_limits<uint32_t>::max();
        uint32_t idxtry = noctants/2;
     ...
     */
    void
    ParTree::getMapping(uint32_t & idx, u32vector & mapper, bvector & isghost) const {
     ...
     */
    void
    ParTree::getMapping(uint32_t & idx, u32vector & mapper, bvector & isghost, ivector & rank) const {
    ...
    /** Get octants with marker different from zero and the related markers and ghostness info.
        ...
        */
    void
    ParTree::getPreMapping(u32vector & idx, vector<int8_t> & markers, vector<bool> & isghost)
    {
        if (m_lastOp != OP_PRE_ADAPT) {
            throw std::runtime_error("Last operation different from preadapt, unable to call getPreMarker function");
        }
     ...
     */
    bool
    ParTree::isNodeOnOctant(const Octant *nodeOctant, uint8_t nodeIndex, const Octant *octant) const {
     ...
     */
    bool
    ParTree::isEdgeOnOctant(const Octant* edgeOctant, uint8_t edgeIndex, const Octant* octant) const {
     ...
     */
    bool
    ParTree::isFaceOnOctant(const Octant* faceOctant, uint8_t faceIndex, const Octant* octant) const {
     ...
     */
    bool
    ParTree::adapt(bool mapper_flag){
     ...
     */
    bool
    ParTree::adaptGlobalRefine(bool mapper_flag) {
     ...
     */
    bool
    ParTree::adaptGlobalCoarse(bool mapper_flag) {
        //TODO recoding for adapting with abs(marker) > 1
        uint32_t nocts0 = getNumOctants();
        vector<Octant>::iterator iter, iterend = m_octree.m_octants.end();
     ...
     */
    int
    ParTree::findOwner(const uint64_t & morton) const {
     ...
     */
    int
    ParTree::getOwnerRank(const uint64_t & globalIndex) const {
     ...
     */
    const u32vector &
    ParTree::getConnectivity(uint32_t idx) const {
     ...
     */
    const u32vector &
    ParTree::getConnectivity(Octant* oct) const {
     ...
     */
    const u32array3 &
    ParTree::getNodeLogicalCoordinates(uint32_t inode) const {
     ...
     */
    darray3
    ParTree::getNodeCoordinates(uint32_t inode) const {
     ...
     */
    const u32vector2D &
    ParTree::getGhostConnectivity() const {
     ...
     */
    const u32vector &
    ParTree::getGhostConnectivity(uint32_t idx) const {
     ...
     */
    const u32vector &
    ParTree::getGhostConnectivity(const Octant* oct) const {
     ...
     */
    void
    ParTree::loadBalance(dvector* weight){
     ...
     */
    void
    ParTree::loadBalance(uint8_t & level, dvector* weight){
     ...
     * processors.
     */
    ParTree::LoadBalanceRanges
    ParTree::evalLoadBalanceRanges(dvector *weights){
     ...
     */
    ParTree::LoadBalanceRanges
    ParTree::evalLoadBalanceRanges(uint8_t level, dvector *weights){
     ...
     */
    ParTree::LoadBalanceRanges
    ParTree::evalLoadBalanceRanges(const uint32_t *updatedPartition){
     ...
     */
    ParTree::ExchangeRanges
    ParTree::evalLoadBalanceSendRanges(const uint32_t *updatedPartition){
     ...
     */
    ParTree::ExchangeRanges
    ParTree::evalLoadBalanceRecvRanges(const uint32_t *updatedPartition){
     ...
     * partition schema B.
     */
    ParTree::PartitionIntersections
    ParTree::evalPartitionIntersections(const uint32_t *partition_A, int rank_A, const uint32_t *partition_B){
     ...
     */
    void
    ParTree::privateLoadBalance(uint32_t* partition){
     ...
     */
    double
    ParTree::levelToSize(uint8_t & level) {
     ...
     */
    Octant&
    ParTree::extractOctant(uint32_t idx) {
     ...
     */
    bool
    ParTree::private_adapt_mapidx(bool mapflag) {
        //TODO recoding for adapting with abs(marker) > 1
     ...
     */
    void
    ParTree::computePartition(uint32_t* partition){
     ...
     */
    void
    ParTree::computePartition(uint32_t* partition, dvector* weight){
        if(m_serial){
     ...
     */
    void
    ParTree::computePartition(uint32_t* partition, uint8_t & level_, dvector* weight) {
    ...
    ParTree::setPboundGhosts() {
        ...
        //for every element it visits the border octants it contains and write them in the sphexa communication structure, DataCommunicator
     ...
     */
    void
    ParTree::balance21(bool const first){
#if SPHEXA_ENABLE_MPI==1
     ...
     * \param[in] filename Name of output file (SPHEXA will add the total number of processes p000# and the current rank s000#).
     */
    void
    ParTree::write(string filename) {
     ...
     */
    void
    ParTree::writeTest(string filename, vector<double> data) {
