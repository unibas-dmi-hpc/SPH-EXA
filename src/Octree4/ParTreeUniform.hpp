#ifndef __SPHEXA_PARTREE_UNIFORM_HPP__
#define __SPHEXA_PARTREE_UNIFORM_HPP__
...
namespace sphexa {
     ...
     *	\ingroup		SPHEXA
     *	\date			24/04/2018
     *	\authors		Danilo Guerrera
     *
     *	\brief ParTreeUniform is an example of user class derived from ParTree to map
     *	ParTree in a uniform (square/cubic) domain.
     *	It takes as input in constructor the coordinates of the origin (X,Y,Z) and the length of the side L.
     *
     *	Class ParTreeUniform has a dimensional parameter int dim and it accepts
     *	only two values: dim=2 and dim=3, for 2D and 3D respectively.
     */
    class ParTreeUniform : public ParTree
    {
        // =================================================================================== //
        // MEMBERS																			   //
        // =================================================================================== //
    ...
    public:
        ...
        ParTreeUniform(std::string logfile = DEFAULT_LOG_FILE, MPI_Comm comm = MPI_COMM_WORLD);
        ParTreeUniform(uint8_t dim, std::string logfile = DEFAULT_LOG_FILE, MPI_Comm comm = MPI_COMM_WORLD);
        ParTreeUniform(double X, double Y, double Z, double L, uint8_t dim = 2, std::string logfile = DEFAULT_LOG_FILE, MPI_Comm comm = MPI_COMM_WORLD);
#else
        ParTreeUniform(std::string logfile = DEFAULT_LOG_FILE);
        ParTreeUniform(uint8_t dim, std::string logfile = DEFAULT_LOG_FILE);
        ParTreeUniform(double X, double Y, double Z, double L, uint8_t dim = 2, std::string logfile = DEFAULT_LOG_FILE);
#endif
...
#endif /* __SPHEXA_PARTREE_UNIFORM_HPP__ */
