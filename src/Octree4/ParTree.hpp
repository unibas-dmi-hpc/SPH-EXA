#ifndef __SPHEXA_PAR_TREE_HPP__
#define __SPHEXA_PAR_TREE_HPP__
...
namespace sphexa {
     ...
     *
     *	\brief ParTree is the user interface class
     *
     ...
     *	process.
     *
     *	Class ParTree has a dimensional parameter int dim and it accepts only two
    ...
    public:
#if SPHEXA_ENABLE_MPI==1
        ParTree(std::string logfile = DEFAULT_LOG_FILE, MPI_Comm comm = MPI_COMM_WORLD);
        ParTree(uint8_t dim, std::string logfile = DEFAULT_LOG_FILE, MPI_Comm comm = MPI_COMM_WORLD);
        ParTree(std::istream &stream, std::string logfile = DEFAULT_LOG_FILE, MPI_Comm comm = MPI_COMM_WORLD);
#else
        ParTree(std::string logfile = DEFAULT_LOG_FILE);
        ParTree(uint8_t dim, std::string logfile = DEFAULT_LOG_FILE);
        ParTree(std::istream &stream, std::string logfile = DEFAULT_LOG_FILE);
#endif
        ~ParTree();

        ParTree(const ParTree & other);

    private:
        ParTree & operator=(const ParTree & other) = delete;
