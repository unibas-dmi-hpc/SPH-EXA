...
struct SedovMPIFileWriter : IFileWriter<Dataset>
{
    ...
    void dumpParticleDataToBinFile(const Dataset& d, const std::string& path) const override
    {
        try
        {
            fileutils::writeParticleDataToBinFileWithMPI(
                d, path, d.x, d.y, d.z, d.vx, d.vy, d.vz, d.h, d.ro, d.u, d.p, d.c, d.grad_P_x, d.grad_P_y, d.grad_P_z);
        }
        catch (MPIFileNotOpenedException& ex)
        {
            if (d.rank == 0) fprintf(stderr, "ERROR: %s. Terminating\n", ex.what());
    ...
    {
        if (d.rank == 0)
            fprintf(stderr, "Warning: dumping checkpoint is not implemented in SedovMPIFileWriter, exiting...\n");
