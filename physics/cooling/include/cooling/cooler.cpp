...
namespace cooling {
    ...
    private:
        ...
        void cool_particle(const T &dt, T &rho, T &u, T &HI_fraction,
        ...
        T energy_to_temperature(const T &dt, T &rho, T &u, T &HI_fraction,
                                T &HII_fraction, T &HM_fraction, T &HeI_fraction, T &HeII_fraction, T &HeIII_fraction,
                                T &H2I_fraction, T &H2II_fraction, T &DI_fraction, T &DII_fraction, T &HDI_fraction,
    ...
    template<typename T>
    void Cooler<T>::Impl::cool_particle(const T &dt, T &rho, T &u, T &HI_fraction,
                                        T &HII_fraction, T &HM_fraction, T &HeI_fraction, T &HeII_fraction,
                                        T &HeIII_fraction,
                                        T &H2I_fraction, T &H2II_fraction, T &DI_fraction, T &DII_fraction,
                                        T &HDI_fraction,
                                        T &e_fraction,
        ...
        if (0 ==
            local_solve_chemistry(&global_values.data, &global_values.rates, &global_values.units, &grackle_fields.data,
                                  dt)) {
    ...
    template<typename T>
    T Cooler<T>::Impl::energy_to_temperature(const T &dt, T &rho, T &u, T &HI_fraction,
                                             T &HII_fraction, T &HM_fraction, T &HeI_fraction, T &HeII_fraction,
                                             T &HeIII_fraction,
                                             T &H2I_fraction, T &H2II_fraction, T &DI_fraction, T &DII_fraction,
                                             T &HDI_fraction,
                                             T &e_fraction, T &metal_fraction, T &volumetric_heating_rate,
                                             T &specific_heating_rate,
                                             T &RT_heating_rate, T &RT_HI_ionization_rate, T &RT_HeI_ionization_rate,
                                             T &RT_HeII_ionization_rate, T &RT_H2_dissociation_rate,
    ...
    void Cooler<T>::init(const double ms_sim, const double kp_sim, const int comoving_coordinates,
        ...
        impl_ptr->init(ms_sim, kp_sim, comoving_coordinates,
                       grackleOptions, t_sim);

    }

    template<typename T>
    void Cooler<T>::cool_particle(const T &dt, T &rho, T &u, T &HI_fraction,
                                  T &HII_fraction, T &HM_fraction, T &HeI_fraction, T &HeII_fraction, T &HeIII_fraction,
                                  T &H2I_fraction, T &H2II_fraction, T &DI_fraction, T &DII_fraction, T &HDI_fraction,
                                  T &e_fraction,
                                  T &metal_fraction, T &volumetric_heating_rate, T &specific_heating_rate,
                                  ...
                                  T &RT_HI_ionization_rate, T &RT_HeI_ionization_rate, T &RT_HeII_ionization_rate,
                                  T &RT_H2_dissociation_rate, T &H2_self_shielding_length) {
        impl_ptr->cool_particle(dt, rho, u, HI_fraction,
                                HII_fraction, HM_fraction, HeI_fraction, HeII_fraction, HeIII_fraction,
                                H2I_fraction, H2II_fraction, DI_fraction, DII_fraction, HDI_fraction, e_fraction,
                                metal_fraction, volumetric_heating_rate, specific_heating_rate, RT_heating_rate,
                                RT_HI_ionization_rate, RT_HeI_ionization_rate, RT_HeII_ionization_rate,
    ...
    template<typename T>
    T Cooler<T>::energy_to_temperature(const T &dt, T &rho, T &u, T &HI_fraction,
                                       T &HII_fraction, T &HM_fraction, T &HeI_fraction, T &HeII_fraction,
                                       T &HeIII_fraction,
                                       T &H2I_fraction, T &H2II_fraction, T &DI_fraction, T &DII_fraction,
                                       T &HDI_fraction, T &e_fraction,
                                       T &metal_fraction, T &volumetric_heating_rate, T &specific_heating_rate,
                                       T &RT_heating_rate,
                                       T &RT_HI_ionization_rate, T &RT_HeI_ionization_rate, T &RT_HeII_ionization_rate,
                                       T &RT_H2_dissociation_rate, T &H2_self_shielding_length) {
        return impl_ptr->energy_to_temperature(dt, rho, u, HI_fraction,
                                               HII_fraction, HM_fraction, HeI_fraction, HeII_fraction, HeIII_fraction,
                                               H2I_fraction, H2II_fraction, DI_fraction, DII_fraction, HDI_fraction,
                                               e_fraction,
                                               metal_fraction, volumetric_heating_rate, specific_heating_rate,
    ...
    void Cooler<T>::Impl::initOptions(const std::map<std::string, std::any> &grackle_options) {
        for (auto [key, value]: grackle_options) {
            try {
                ...
                if (key == ("use_grackle"))
                    global_values.data.use_grackle = std::any_cast<int>(value);
                if (key == ("with_radiative_cooling"))
                    global_values.data.with_radiative_cooling = std::any_cast<int>(value);
                if (key == ("primordial_chemistry"))
                    global_values.data.primordial_chemistry = std::any_cast<int>(value);
                if (key == ("h2_on_dust"))
                    global_values.data.h2_on_dust = std::any_cast<int>(value);
                if (key == ("metal_cooling"))
                    global_values.data.metal_cooling = std::any_cast<int>(value);
                if (key == ("cmb_temperature_floor"))
                    global_values.data.cmb_temperature_floor = std::any_cast<int>(value);
                if (key == ("UVbackground"))
                    global_values.data.UVbackground = std::any_cast<int>(value);
                ...
                if (key == ("Gamma"))
                    global_values.data.Gamma = std::any_cast<int>(value);
                if (key == ("three_body_rate"))
                    global_values.data.three_body_rate = std::any_cast<int>(value);
                if (key == ("cie_cooling"))
                    global_values.data.cie_cooling = std::any_cast<int>(value);
                ...
                if (key == ("Compton_xray_heating"))
                    global_values.data.Compton_xray_heating = std::any_cast<int>(value);
                ...
                if (key == ("H2_self_shielding"))
                    global_values.data.H2_self_shielding = std::any_cast<int>(value);
                if (key == ("dust_chemistry"))
                    global_values.data.dust_chemistry = std::any_cast<int>(value);
            }
