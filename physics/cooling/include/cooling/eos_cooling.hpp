...
auto coolingTimestep(size_t first, size_t last, Dataset& d, Cooler& cooler, Chem& chem)
{
    ...
    //    {
    //        const T cooling_time = cooler.cooling_time(d.rho[i], d.u[i], cstone::getPointers(get<CoolingFields>(chem),
    //        i)); minTc                = std::min(std::abs(cooler.ct_crit * cooling_time), minTc);
    //    }
    //    return minTc;
}
