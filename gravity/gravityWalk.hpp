...
template <typename T>
void treeWalkRef(const GravityTree<T> &gtree, const int i, const T *xi, const T *yi, const T *zi, const T *hi, const T *hj, const T *mj, T *fx,
                 T *fy, T *fz, T *ugrav, RankToParticles &particlesForRemoteGravCalculations)
{
    ...
    for (auto gnode: gtree.nodeData)
    {
        ...
        if (d1 <= dc && d2 <= dc && d3 <= dc) // intersecting
        {
            ...
            {
                /* TODO: get all children of this node
...
void gravityTreeWalkImpl(const sphexa::Task &t, const GravityTree<T> &gtree, Dataset &d, RankToParticles &particlesForRemoteGravCalculations)
{
...
#pragma omp parallel for
    for (size_t pi = 0; pi < n; ++pi)
    {
        ...
        treeWalkRef(gtree, i, xi, yi, zi, hi, hj, mj, fx, fy, fz, ugrav, particlesForRemoteGravCalculations);
    }
}

template <typename T, class Dataset>
RankToParticles gravityTreeWalk(const std::vector<sphexa::Task> &taskList, const GravityTree<T> &gtree, Dataset &d)
{
    RankToParticles particlesForRemoteGravCalculations;

    for (const auto &task : taskList)
    {
        gravityTreeWalkImpl<T>(task, gtree, d, particlesForRemoteGravCalculations);
    }
