...
def splitDatasetSingleFile(input_file, step, file_ind, total_file_inds, output_initial):
    ...
    vx_dset = group.create_dataset('vx', (curr_slice_size,), chunks=(chunk_size,), maxshape=(None,), dtype='f4')
    vy_dset = group.create_dataset('vy', (curr_slice_size,), chunks=(chunk_size,), maxshape=(None,), dtype='f4')
    vz_dset = group.create_dataset('vz', (curr_slice_size,), chunks=(chunk_size,), maxshape=(None,), dtype='f4')
    h_dset = group.create_dataset('h', (curr_slice_size,), chunks=(chunk_size,), maxshape=(None,), dtype='f4')
    ...
    for chunk in vy_dset.iter_chunks():
        start = chunk[0].start
        stop = chunk[0].stop
        vy_dset[chunk] = np.array([h5step["vy"][curr_start_ind+start:curr_start_ind+stop]])

    for chunk in vz_dset.iter_chunks():
        start = chunk[0].start
        stop = chunk[0].stop
        vz_dset[chunk] = np.array([h5step["vz"][curr_start_ind+start:curr_start_ind+stop]])

    for chunk in h_dset.iter_chunks():
        start = chunk[0].start
        stop = chunk[0].stop
        h_dset[chunk] = np.array([h5step["h"][curr_start_ind+start:curr_start_ind+stop]])
