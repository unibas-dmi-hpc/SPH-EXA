...
CXXFLAGS += $(RELEASE)
NVCCARCH := sm_60
NVCCFLAGS := -std=c++11 --expt-relaxed-constexpr -rdc=true -arch=$(NVCCARCH)
...
omp+cuda: $(BUILDDIR)/cuda_no_mpi.o $(CUDA_OBJS)
	...
	$(CXX) $(CXXFLAGS) -o $(BINDIR)/$@.app cudalinked.o $+ -lcudadevrt -lcudart
...
mpi+omp+cuda: $(BUILDDIR)/cuda_mpi.o $(CUDA_OBJS)
	...
	$(MPICXX) $(CXXFLAGS) -o $(BINDIR)/$@.app cudalinked.o $+ -lcudadevrt -lcudart
