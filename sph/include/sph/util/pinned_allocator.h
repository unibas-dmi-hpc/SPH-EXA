 ...
 *  \brief An allocator which creates new elements in "pinned" memory with \p hipMallocHost
 */
 ...
 *  that employs \c hipMallocHost for allocation.
 *
...
public:
    ...
    inline pointer allocate(size_type cnt, const_pointer = 0)
    {
        ...
        pointer     result(0);
        hipError_t error = hipMallocHost(reinterpret_cast<void**>(&result), cnt * sizeof(value_type));
     ...
     */
    inline size_type max_size() const { return (std::numeric_limits<size_type>::max)() / sizeof(T); } // end max_size()
