...
int main(){
 using T = double;
  ...
  stir_init(stAmpl, stMode, stNModes, stSolWeight, stSolWeightNorm, stOUVar, stDecay,
     Lbox, Lbox , Lbox, stMaxModes, stEnergy ,stStirMax, stStirMin, dim, stSpectForm);
  ...
  //Problem!!!, st_OUphases, st_nmodes, st_OUvar & st_seed must be output too, need to store values between iterations
  ...
  st_calcAccel(0,npart,dim,xCoord,yCoord,zCoord,accx,accy,accz,stNModes,stMode,st_aka,st_akb,stAmpl,stSolWeightNorm);
